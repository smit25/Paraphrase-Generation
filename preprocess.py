...
def main(params):
    ...
    with open('vocab.txt', 'wb') as vocab_save:
        pickle.dump(final_list, vocab_save)
        vocab_save.close()
    
    itow = {i+1:w for i,w in enumerate(final_list)} # 1-indexed vocab translation table
    wtoi = {w:i+1 for i,w in enumerate(final_list)} # Bag of words model
    #wtoi['UNK'] = 0
    print('len of wtoi', len(wtoi))
    print('len of itow', len(itow))
