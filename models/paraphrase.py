...
class Paraphrase(nn.module):
    ...
    def forward(self, phrase, sim_phrase, out):
        ...
        encoded = self.encoder(phrase)
        sim_phrase, decoded = self.decoder(phrase, encoded, sim_phrase)

        enc_sim_phrase = self.enc_lin(
                self.enc_rnn(
                    self.enc_emb_layer(utils.one_hot(sim_phrase,self.vocab_sz)))[1])
        
        enc_out = self.enc_lin(self.enc_rnn(self.enc_emb_layer(torch.exp(decoded)))[1])

        enc_out.squeeze_(0)
        enc_sim_phrase.squeeze_(0)
        return decoded, enc_out, enc_sim_phrase
